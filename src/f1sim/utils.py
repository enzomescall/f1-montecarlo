...
class TireSpec:
    name: str
    deg_s_per_lap: float         # linear base (can curve later)
    deg_curve: float = 0.0       # >0 for accelerating wear (non-linear)
...
class DriverCar:
    code: str
    base_pace: float         # s/lap @ fuel=0, fresh soft, clean air
    fuel_s_per_kg: float     # s/lap per kg fuel penalty
    minisector_noise: float  # s/ms stdev
    lap_variance: float      # s/lap stdev shock
    pit_loss_s: float        # total pit in+out+service (no refueling in modern F1)

@dataclass(order=True)
class Event:
    t: float
    car_idx: int
    kind: str         # "advance", "pit_done", "sc_end", etc.
...
class CarState:
    ...
    last_detection_t: float = 1e9
    track_pos: float = 0.0     # lap + ms/N_ms for ordering
...
class RaceSim:
    def __init__(
        ...
        overtake_alpha: float = 2.8,
        overtake_beta: float = 1.0,
        overtake_gamma: float = 0.5,
        overtake_delta: float = 1.8,
    ):
        self.rng = random.Random(seed)
        self.tr = track
        self.field = [CarState(d, tire_specs[strat_plan[d.code][0][1]], fuel_kg=105.0)
                      for d in grid]  # ~105kg start, tweak per track
        self.race_laps = race_laps
        self.tires = tire_specs
        ...
        self.params = dict(
            ...
            a=overtake_alpha, b=overtake_beta, c=overtake_gamma, d=overtake_delta
        )
    ...
    def _end_sc(self, now: float):
        self.flags["SC"] = False
        self.flags["VSC"] = False

    def _race_over(self) -> bool:
        # done when all non-retired cars reached final lap
        return all(c.retired or c.lap >= self.race_laps for c in self.field)

# ---------- Example usage (toy) ----------

def example():
    # 60 minisectors with simple weights; mark straights/aero crudely
    N = 60
    weights = [1.0/N]*N
    straights = set(range(5,10)) | set(range(25,30)) | set(range(45,50))
    aero = set(i for i in range(N) if i not in straights)

    track = Track(
        name="Toyring",
        minisector_weights=weights,
        drs_zones=[(6,10), (26,30), (46,50)],
        drs_detection_ms=[4,24,44],
    ...
    tires = {
        "S": TireSpec("S", deg_s_per_lap=0.04, deg_curve=0.015),
        "M": TireSpec("M", deg_s_per_lap=0.03, deg_curve=0.010),
        "H": TireSpec("H", deg_s_per_lap=0.02, deg_curve=0.006),
    }

    grid = [
        DriverCar("VER", base_pace=80.5, fuel_s_per_kg=0.030, minisector_noise=0.001, lap_variance=0.08, pit_loss_s=22.0),
        DriverCar("HAM", base_pace=80.9, fuel_s_per_kg=0.030, minisector_noise=0.001, lap_variance=0.09, pit_loss_s=22.0),
        DriverCar("NOR", base_pace=81.1, fuel_s_per_kg=0.031, minisector_noise=0.001, lap_variance=0.09, pit_loss_s=22.0),
        DriverCar("LEC", base_pace=81.2, fuel_s_per_kg=0.031, minisector_noise=0.001, lap_variance=0.09, pit_loss_s=22.0),
        # ... add more up to 20
    ]

    # simple 1-stop: S->M around lap 20
    strat = {d.code: [(20, "M")] for d in grid}

    sim = RaceSim(track, grid, race_laps=57, tire_specs=tires, strat_plan=strat, seed=42)
    result = sim.run()
    for c in sorted(result, key=lambda x: x.t):
        status = "RET" if c.retired else f"{c.t:.1f}s"
        print(f"{c.driver.code}: L{c.lap}  {status}")
